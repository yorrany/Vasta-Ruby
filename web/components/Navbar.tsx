...
export function Navbar() {
  ...
  return (
    <header className="fixed top-0 z-50 w-full border-b border-slate-800/60 bg-vasta-bg/80 backdrop-blur-md">
              ...
              {user ? (
                <div className="relative">
                  <button 
                    ...
                    className="flex items-center gap-2 rounded-full border border-slate-800 bg-slate-900/50 p-1.5 transition-colors hover:border-slate-700"
                  >
                  ...
                  {/* User Dropdown */}
                  {isUserMenuOpen && (
                    <div className="absolute right-0 mt-3 w-56 origin-top-right rounded-2xl border border-slate-800 bg-slate-900 p-2 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center gap-6">
                  <button
                    ...
                    className="hidden text-sm font-bold text-white transition-colors hover:text-vasta-primary sm:block"
                  >
                    Entrar
                  </button>
                  <button
                    onClick={() => openAuth('signup')}
                    className="group hidden items-center gap-2 rounded-xl bg-white px-6 py-2.5 text-sm font-bold text-slate-950 transition-all hover:bg-slate-200 sm:flex shadow-lg shadow-white/5"
                  >
                    Começar grátis
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="fixed inset-x-0 top-[73px] z-40 h-screen border-t border-slate-800 bg-vasta-bg/95 p-6 backdrop-blur-xl animate-in slide-in-from-top duration-300 md:hidden">
          <nav className="flex flex-col gap-6">
            ...
            <div className="mt-4 flex flex-col gap-4 border-t border-slate-800 pt-8">
              {!user && (
                <>
                  <button 
                    onClick={() => openAuth('login')}
                    className="flex items-center justify-between rounded-xl border border-slate-800 p-4 text-white"
                  >
                  ...
                  <button 
                    onClick={() => openAuth('signup')}
                    className="flex items-center justify-between rounded-xl bg-white p-4 font-bold text-black"
                  >
                    Começar agora
